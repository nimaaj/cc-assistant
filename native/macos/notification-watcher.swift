...
while true {
    autoreleasepool {
        ...
        for window in windows {
            ...
            let meaningful = values.filter { value in
                !["Notification Center", "Close", "Actions"].contains(value)
            }
            guard !meaningful.isEmpty else { continue }
            let sourceApp = meaningful.count > 1 ? meaningful[0] : "macOS"
            let title = meaningful.count > 1 ? meaningful[1] : meaningful[0]
            let body = meaningful.dropFirst(min(2, meaningful.count)).joined(separator: " · ")
