...
function nextRun(triggerKind: Schedule["triggerKind"], trigger: JsonRecord, now = Date.now()): string | null {
  ...
  if (triggerKind === "system_notification") {
    if (trigger.cooldownMs !== undefined) {
      const cooldownMs = Number(trigger.cooldownMs);
      if (!Number.isInteger(cooldownMs) || cooldownMs < 1_000 || cooldownMs > 31_536_000_000) {
        throw new Error("A notification trigger cooldownMs must be between 1000 and 31536000000");
      }
    }
    return null;
  }
...
export class AssistantRepository {
  ...
  updateSchedule(id: string, rawInput: unknown): Schedule {
    ...
    if (!current) throw new ScheduleNotFoundError(`Schedule ${id} was not found`);
    if (input.expectedRevision !== current.revision) throw new ScheduleRevisionConflictError(`Schedule ${id} has changed since it was loaded`);
    ...
    const updated = ScheduleSchema.parse({
      ...current, ...input,
      name: validated.name, enabled: validated.enabled,
      triggerKind: validated.triggerKind, trigger: validated.trigger,
      actionKind: validated.actionKind, action: validated.action,
    ...
    const result = this.#db.prepare(`UPDATE schedules SET name=?, enabled=?, trigger_kind=?, trigger_json=?,
      action_kind=?, action_json=?, next_run_at=?, last_run_at=?, updated_at=?, revision=? WHERE id=? AND revision=?`)
      .run(updated.name, Number(updated.enabled), updated.triggerKind, JSON.stringify(updated.trigger),
        ...
        updated.updatedAt, updated.revision, id, current.revision);
    if (result.changes !== 1) throw new ScheduleRevisionConflictError(`Schedule ${id} changed during the update`);
  ...
  getBrowserJobForRun(runId: string): BrowserJob | undefined {
    const row = this.#db.prepare("SELECT * FROM browser_jobs WHERE run_id=? ORDER BY created_at DESC LIMIT 1").get(runId) as Row | undefined;
    return row ? mapBrowserJob(row) : undefined;
  }

  listBrowserJobs(status?: BrowserJob["status"]): BrowserJob[] {
    const rows = status ? this.#db.prepare("SELECT * FROM browser_jobs WHERE status=? ORDER BY created_at DESC").all(status) :
      this.#db.prepare("SELECT * FROM browser_jobs ORDER BY created_at DESC").all();
    return (rows as Row[]).map(mapBrowserJob);
  }
  ...
  requeueInterruptedBrowserJobs(): number {
    const result = this.#db.prepare("UPDATE browser_jobs SET status='queued', claimed_at=NULL WHERE status='claimed'").run();
    return Number(result.changes);
  }

  completeBrowserJob(id: string, result: unknown, error?: string): BrowserJob {
    const current = this.getBrowserJob(id);
    if (!current) throw new Error(`Browser job ${id} was not found`);
  ...
  cancelBrowserJobsForRun(runId: string): void {
    const now = new Date().toISOString();
    this.#db.prepare(`UPDATE browser_jobs SET status='cancelled',error='Run cancelled by the user',completed_at=?
      WHERE run_id=? AND status IN ('queued','claimed')`).run(now, runId);
  }
