...
function commandEnvironment(additions: Record<string, string>): NodeJS.ProcessEnv {
  const inherited = Object.fromEntries(Object.entries(process.env).filter(([name]) =>
    !SECRET_ENV_NAME.test(name) && (!EXECUTION_CONTROL_ENV_NAME.test(name) || name === "PATH")));
  return { ...inherited, ...additions };
}

function agentEnvironment(useClaudeLogin: boolean): NodeJS.ProcessEnv {
...
export class ExecutionService {
  ...
  proposeCommand(rawInput: ProposeCommandInput): { run: Run; approval: Approval } {
    ...
    if (input.executable.includes("/") && !isAbsolute(input.executable)) {
      throw new ExecutionInputError("Executable paths must be absolute or resolved through PATH");
    }
    const forbidden = Object.keys(input.env).filter((name) => SECRET_ENV_NAME.test(name) || EXECUTION_CONTROL_ENV_NAME.test(name));
    if (forbidden.length > 0) {
      throw new ExecutionInputError(`Secret-like or execution-control environment variables are not accepted: ${forbidden.join(", ")}`);
    }
    const run = this.#repository.createRun({
      ...(input.taskId !== undefined ? { taskId: input.taskId } : {}),
      kind: "command",
  ...
  cancel(runId: string): Run {
    ...
    if (!run) throw new RunNotFoundError(`Run ${runId} was not found`);
    ...
    if (active?.child) {
      ...
      const forceTimer = setTimeout(() => { if (child.exitCode === null) child.kill("SIGKILL"); }, 2_000);
      forceTimer.unref();
    }
    ...
    this.#repository.expireApprovalsForRun(runId, "Run cancelled by the user");
  ...
  #allowedDirectory(rawPath: string): string {
    ...
    try {
      candidate = realpathSync(resolve(rawPath));
    } catch {
      throw new ExecutionInputError(`Working directory does not exist: ${resolve(rawPath)}`);
    }
    ...
    if (!roots.some((root) => isWithin(candidate, root))) {
      throw new ExecutionInputError(`Working directory is outside allowed roots: ${candidate}`);
    }
    return candidate;
  }
    ...
    try {
      ...
      const stream = this.#agentQuery({
        ...
        options: {
          ...
          env: agentEnvironment(this.#config.managedAgent?.useClaudeLogin ?? true),
          effort: input.effort,
          maxTurns: input.maxTurns,
