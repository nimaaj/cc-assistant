import { z } from "zod";
import type { MemoryRecord } from "@cc-assistant/shared";
...
const metadataKeys = [
  "cc_memory_format", "id", "slug", "title", "summary", "kind", "tags", "aliases",
  "project", "status", "revision", "source_type", "source_uri", "source_ref", "captured_at",
] as const;

export function serializeMemoryMarkdown(memory: MemoryRecord): string {
  ...
  const frontmatter = metadataKeys.map((key) => `${key}: ${JSON.stringify(values[key])}`).join("\n");
...
export function parseMemoryMarkdown(path: string, content: string): ImportedMemory {
  ...
  for (const line of (match[1] ?? "").split("\n")) {
    ...
    try {
      rawMetadata[key] = JSON.parse(line.slice(separator + 1).trim()) as unknown;
    } catch {
      throw new Error(`Frontmatter field ${key} must contain a JSON value`);
    }
  }

  return ImportedMemorySchema.parse({
