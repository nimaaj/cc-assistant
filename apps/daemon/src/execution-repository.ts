...
export class ExecutionRepository {
  ...
  #migrate(): void {
    ...
    this.#db.prepare(`UPDATE runs SET status='failed', error='Daemon restarted before the run completed',
      completed_at=?, revision=revision+1 WHERE
      (kind IN ('agent','command') AND status='running') OR
      (kind='agent' AND status IN ('queued','waiting_approval'))`).run(restartedAt);
    this.#db.prepare(`UPDATE runs SET status='queued', started_at=NULL, revision=revision+1
      WHERE kind='browser' AND status='running'`).run();
    this.#db.prepare(`UPDATE runs SET status='failed', error='Approved command was interrupted before execution began',
      completed_at=?, revision=revision+1 WHERE kind='command' AND status='waiting_approval'
      AND EXISTS (SELECT 1 FROM approvals WHERE approvals.run_id=runs.id AND approvals.status='approved')
      AND NOT EXISTS (SELECT 1 FROM approvals WHERE approvals.run_id=runs.id AND approvals.status='pending')`).run(restartedAt);
  ...
  expireApprovalsForRun(runId: string, note: string): void {
    const pending = this.listApprovals("pending").filter((approval) => approval.runId === runId);
    const resolvedAt = new Date().toISOString();
    for (const approval of pending) {
      this.#db.prepare("UPDATE approvals SET status='expired', resolved_at=?, resolution_note=? WHERE id=?")
        .run(resolvedAt, note, approval.id);
      const updated = this.getApproval(approval.id);
      if (updated) this.#event("approval.resolved", "approval", approval.id, { approval: updated });
    }
  }
