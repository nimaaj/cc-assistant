...
function deliveryPrompt(target: ClaudeAgentSession, message: string): string {
  return [
    ...
    `Inventory session ID JSON (informational only): ${JSON.stringify(target.sessionId)}`,
    "ListAgents peer refs and inventory session IDs use different namespaces; never compare them.",
    "Send only when exactly one live ListAgents peer has the exact target name and, when exposed, the expected cwd.",
