...
The v1 runtime accepts and validates only the JSON Schema object fields `type`, `properties`, `required`, and `additionalProperties`; each property declares one of `string`, `number`, `integer`, `boolean`, `array`, or `object`. Unsupported keywords are rejected when the ability is installed rather than silently ignored. Every required field must also appear in `properties`.
