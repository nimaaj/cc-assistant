...
| Web UI | Default simplified dispatcher/status-pane view plus the complete workspace for tasks, reminders, managed runs, exact approval payloads, notifications, sessions, memory, Calendar/Slack, triggers, commands, abilities, clipboard, browser-worker state, and State studio | Static-shell/asset/auth boundary tests, Web API contract tests, production Vite build, and live DOM/visual smoke of both tabs and horizontal pane expansion | Verified locally |
