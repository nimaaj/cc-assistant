...
| Spin off agents | Claude Agent SDK execution service, persisted runs/logs, permission callback, limits, cancellation, optional worktree | Permission/recovery/worktree tests plus authenticated Claude Code-login run `b32f158b-77ca-4e2d-aebc-68666c2e6096` | Verified locally and live |
...
| Check Google Calendar without its API | Approved/read-only browser job contracts and bounded Claude-in-Chrome worker with visible-event, date/view, and verified-create instructions | Shared validation, injected Agent SDK stream/tool-isolation test, and signed-in read-only Calendar smoke on 2026-09-17 | Read path verified live; approved create remains intentionally untested |
...
| Read clipboard image | macOS JXA and Linux Wayland/X11 adapters; MCP image result and CLI file output | Injected macOS PNG parsing and Linux fallback tests | Implemented; physical macOS clipboard check pending |
| Slack without its API | Claude-in-Chrome read unreads/channel, exact channel targeting, approved exact send, and verification requirement | Shared validation, injected Agent SDK stream/tool-isolation test, and signed-in unread smoke on 2026-09-17 | Read path verified live; approved send remains intentionally untested |
...
| Time/system-notification triggers | Validated trigger/action contracts, durable next-run state, cooldown, in-process loop guard, macOS Accessibility watcher | Notification match/cooldown test; Swift source present | Implemented; Notification Center check requires macOS Accessibility permission |
| Web UI | Tasks, reminders, managed runs, exact approval payloads, notifications, sessions, memory workspace, Claude-in-Chrome worker state | Production Vite build; local dashboard smoke | Verified locally |
...
1. Run `pnpm smoke:browser:e2e -- --help`, then invoke its guarded write mode with a disposable event and safe test channel. The signed-in read paths already passed; writes remain manual because they create external side effects.
...
All three local gates pass after the Claude-in-Chrome replacement. The MCP smoke requires a running local daemon. Signed-in Calendar and Slack reads and a bounded managed Claude run passed; account writes and macOS-only behavior remain explicit manual checks because they create external side effects or require unavailable hardware/permissions.
