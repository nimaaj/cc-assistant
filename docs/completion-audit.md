...
| Control and orchestrate Claude Code sessions | Supported agent-view JSON/log interfaces; approval-backed dispatch, message, continue, stop, respawn, and safe remove; manual/auto/bypass dispatch; native background controller plus `claude attach`; MCP, CLI, and dashboard controls | Session-control and launcher tests, type checks, production build, and live `claude agents --json --all` inventory | Verified locally |
...
| Web UI | Default simplified dispatcher with an animated zoomable React Flow canvas, whole-card dragging, desktop-style multi-selection, right-click menus, embedded controls and diagnostics, durable arbitrary pane positions, grid/status/category/time auto-arrangement, automatic drag-to-group folders, virtual completed-item Archive, reversible all-item Trash, top-layer hover expansion, controller permission selector, and six themes, plus the complete workspace | Static-shell/asset/auth boundary tests, archive/auto-arrangement tests, folder/layout/trash repository and API tests, production Vite build, and live DOM/visual smoke of canvas interactions | Verified locally |
