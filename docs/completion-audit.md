...
| Web UI | Default simplified dispatcher with animated React Flow canvas, pan/Shift-click selection, selection- and branch-aware context menus, prompt/creator provenance badges and curved graph edges, persistent auto-arrangement, folders/archive/trash, diagnostics/themes, runtime recipe editor and controls, plus movable/collapsible/minimizable transcript windows | Static-shell/asset/auth tests, archive/auto-arrangement/graph-cycle tests, folder/layout/trash/provenance API tests, type check, production build, and live browser verification of Automatic delivery plus a two-item branch menu (`Create folder from branch`, `Move branch to Trash`) on 2026-09-23 | Verified locally and live |
