...
| Control and orchestrate Claude Code sessions | Supported agent-view JSON/log interfaces; approval-backed dispatch, message, continue, stop, respawn, and safe remove; MCP, CLI, and dashboard controls | Session-control service tests, type checks, production build, and live `claude agents --json --all` inventory | Verified locally |
