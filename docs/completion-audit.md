...
| Control and orchestrate Claude Code sessions | Supported agent-view JSON/log interfaces; approval-backed dispatch, message, continue, stop, respawn, and safe remove; manual/auto/bypass dispatch; default tmux recipe with fixed dispatcher lead and Claude team panes; repair/relaunch controls; immediate audited terminal attachment; native background attachment | Session-control, controller-launcher, recipe validation, type checks, build, live `claude agents --json --all` inventory, live runtime-control/transcript UI smoke, and 2026-09-23 terminal-launch verification with successful run records and no pending approval | UI and contracts verified; full recipe-created tmux/Claude smoke pending |
| Natural-language dispatcher | Simplified single-input UI, newest-live-controller selection, versioned dispatch envelope with exact origin, structured subtask/result prompt contract, manual delivery approval plus narrowly auto-resolved Automatic/Bypass delivery, and approval-backed scheduled or notification-triggered delivery | Dispatcher selection/envelope, provenance, automation, and web-client tests; on 2026-09-23 a harmless Automatic-mode request entered `running` with no pending approval and displayed its exact prompt/creator trace | Verified locally and live |
...
| Web UI | Default simplified dispatcher with animated React Flow canvas, pan/Shift-click selection, selection- and branch-aware context menus, prompt/creator provenance badges and curved graph edges, persistent measured-size auto-arrangement, folders/archive/trash, diagnostics/themes, runtime recipe editor and controls, plus movable/collapsible/minimizable transcript windows | Static-shell/asset/auth tests, archive/auto-arrangement/graph-cycle tests, folder/layout/trash/provenance API tests, type check, production build, and 2026-09-23 live verification of Automatic delivery, branch menus, a dispatcher-created task with prompt/creator lineage, a visible dispatcher edge, and zero overlaps in Grid/Status/Category/Newest modes while an item was expanded | Verified locally and live |
