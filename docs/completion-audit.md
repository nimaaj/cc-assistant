...
| Web UI | Default simplified dispatcher/status-pane view with durable drag-and-drop folders, recoverable task trash, and six themes, plus the complete workspace for tasks, reminders, managed runs, exact approval payloads, notifications, sessions, memory, Calendar/Slack, triggers, commands, abilities, clipboard, browser-worker state, and State studio | Static-shell/asset/auth boundary tests, folder repository/API tests, production Vite build, and live DOM/visual smoke of tabs, pane expansion, drag/drop, and themes | Verified locally |
