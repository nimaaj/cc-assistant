...
Tags and aliases are lower-cased, whitespace-normalized, deduplicated, and sorted.
...
SQLite FTS5 indexes page title, aliases, summary, body, and tags in the same transaction as creation or editing. Archived pages stay indexed so an explicit archived search remains possible, but list, search, and recall omit them by default. Search text is normalized into quoted lexical tokens before it reaches `MATCH`, so punctuation, quotes, operators, or an empty string cannot produce a raw FTS syntax error. Empty or punctuation-only queries return recently updated pages.
...
- `GET /api/memories`: list pages. Supports `limit`, `offset`, `status`, `project`, `kind`, `tag`, and `includeArchived`.
- `GET /api/memories/search`: lexical search. Supports `q`, `limit`, `status`, `project`, `kind`, `tag`, and `includeArchived`.
...
- `POST /api/memories/recall`: return a compact set of relevant active pages for `query` and optional filters.
- `GET /api/memories/:id-or-slug`: read one page.
- `GET /api/memories/:id-or-slug/links`: read outgoing links and backlinks.
...
- `PATCH /api/memories/:id-or-slug`: edit with required `expectedRevision`.
- `POST /api/memories/:id-or-slug/archive`: archive with required `expectedRevision`.
...
The same workflow with npm is `npm run cca -- memory export ...` and
`npm run cca -- memory import ...`.
...
This interchange layer incorporates the reusable functionality from the former
`cc-knowledge-base` prototype directly into this repository. cc-assistant has no runtime or
installation dependency on that checkout. Migrate selected material through an export and dry-run
import; never replace `.data/assistant.sqlite` with another project's database.
